...
class IntentionClassifier(nn.Module):
    def __init__(self, args):
        ...
        self.full_connection = nn.Linear(args.hidden_size * 3,
                                         args.topic_class_num)

    def forward(self, context_rep, topic_rep, profile_pooled):
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.context_lstm = nn.LSTM(input_size=self.args.embedding_dim,
                                    hidden_size=self.args.hidden_size,
                                    num_layers=self.args.num_layers,
                                    dropout=self.args.dropout_hidden,
                                    batch_first=True)

        self.topic_lstm = nn.LSTM(input_size=self.args.embedding_dim,
                                  hidden_size=self.args.hidden_size,
                                  num_layers=self.args.num_layers,
                                  dropout=self.args.dropout_hidden,
                                  batch_first=True)

        self.profile_lstm = nn.LSTM(input_size=self.args.embedding_dim,
                                    hidden_size=self.args.hidden_size,
                                    num_layers=self.args.num_layers,
                                    dropout=self.args.dropout_hidden,
                                    batch_first=True)
        ...
        if args.init_from_fineturn:
            self.load_state_dict(torch.load(join(self.args.lstm_path, 'model')))
    ...
    def forward(self, x):
        ...
        init_h0 = (torch.zeros(self.args.num_layers, batch_size * sent_num, self.args.hidden_size).to(self.args.device),
                   torch.zeros(self.args.num_layers, batch_size * sent_num, self.args.hidden_size).to(self.args.device))
        ...
        # profile_rep: [batch, sent_num, hidden_size]
        profile_rep = profile_rep.view(batch_size, sent_num, -1)
        # batch, hidden_size
        profile_rep = torch.mean(profile_rep, dim=1)

        # out_topic_id:[batch_size, topic_num_class]
        out_topic_id = self.intention_classifier(context_rep, topic_rep, profile_rep)
